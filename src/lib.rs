...
use std::{cmp::min, collections::HashSet, fs::create_dir_all, fs::File};
...
pub fn read_yaml_file(path: &str) -> Result<HashMap<String, Vec<Image>>, YamlReadError> {
    let contents = fs::read_to_string(path)?;
    let ht: HashMap<String, Vec<Image>> = serde_yaml::from_str(&contents)?;
    Ok(ht)
}
